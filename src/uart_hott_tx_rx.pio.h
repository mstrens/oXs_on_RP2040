...
static inline pio_sm_config hott_uart_tx_program_get_default_config(uint offset) {
    pio_sm_config c = pio_get_default_sm_config();
    sm_config_set_wrap(&c, offset + hott_uart_tx_wrap_target, offset + hott_uart_tx_wrap);
...
static inline void hott_uart_tx_program_init(PIO pio, uint sm, uint offset, uint pin_tx, uint baud, bool invert) {
    ...
    pio_sm_config c = hott_uart_tx_program_get_default_config(offset);
...
static inline void hott_uart_tx_program_stop(PIO pio, uint sm, uint pin_tx) {
...
static inline void hott_uart_tx_program_start(PIO pio, uint sm, uint pin_tx , bool invert ) {
...
static inline void hott_uart_tx_program_putc(PIO pio, uint sm, char c) {
    pio_sm_put_blocking(pio, sm, (uint32_t)c);
}
static inline void hott_uart_tx_program_puts(PIO pio, uint sm, const char *s) {
    while (*s)
        hott_uart_tx_program_putc(pio, sm, *s++);
}
...
#define hott_uart_rx_wrap_target 0
#define hott_uart_rx_wrap 8

static const uint16_t hott_uart_rx_program_instructions[] = {
            //     .wrap_target
...
#if !PICO_NO_HARDWARE
static const struct pio_program hott_uart_rx_program = {
    .instructions = hott_uart_rx_program_instructions,
    .length = 9,
    .origin = -1,
};

static inline pio_sm_config hott_uart_rx_program_get_default_config(uint offset) {
    pio_sm_config c = pio_get_default_sm_config();
    sm_config_set_wrap(&c, offset + hott_uart_rx_wrap_target, offset + hott_uart_rx_wrap);
    return c;
}

#include "hardware/gpio.h"
#include "hardware/clocks.h"
static inline void hott_uart_rx_program_init(PIO pio, uint sm, uint offset, uint pin_rx, uint baud , bool invert) {
    ...
    pio_sm_config c = hott_uart_rx_program_get_default_config(offset);
...
static inline void hott_uart_rx_program_stop(PIO pio, uint sm, uint pin_rx) {
    pio_sm_set_enabled(pio, sm, false); // disable sm.    
}
static inline void hott_uart_rx_program_restart(PIO pio, uint sm, uint pin_rx , bool invert) {
    pio_sm_set_pindirs_with_mask(pio, sm, 0 , 1u << pin_rx); // set pin as input
    if (invert) {
        gpio_set_inover(pin_rx,  GPIO_OVERRIDE_INVERT) ; // added by ms to manage inverted UART from Sport
        gpio_pull_down(pin_rx); // changed by MS
