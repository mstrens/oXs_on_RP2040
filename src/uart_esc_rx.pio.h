...
#define esc_uart_rx_wrap_target 0
#define esc_uart_rx_wrap 8

static const uint16_t esc_uart_rx_program_instructions[] = {
            //     .wrap_target
...
#if !PICO_NO_HARDWARE
static const struct pio_program esc_uart_rx_program = {
    .instructions = esc_uart_rx_program_instructions,
    .length = 9,
    .origin = -1,
};

static inline pio_sm_config esc_uart_rx_program_get_default_config(uint offset) {
    pio_sm_config c = pio_get_default_sm_config();
    sm_config_set_wrap(&c, offset + esc_uart_rx_wrap_target, offset + esc_uart_rx_wrap);
    return c;
}

#include "hardware/gpio.h"
#include "hardware/clocks.h"
static inline void esc_uart_rx_program_init(PIO pio, uint sm, uint offset, uint pin_rx, uint baud , bool invert) {
    ...
    pio_sm_config c = esc_uart_rx_program_get_default_config(offset);
