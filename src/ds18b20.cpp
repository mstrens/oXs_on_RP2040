...
void onewire_set_fifo_thresh(struct owobj *owobj, uint thresh) {
    ...
    newThresh  = ((thresh & 0x1fu) << PIO_SM0_SHIFTCTRL_PUSH_THRESH_LSB) |
    ...
    if( old != newThresh ){
        ...
        hw_write_masked(&pio->sm[sm].shiftctrl, newThresh,
...
void onewire_wait_for_idle(struct owobj *owobj) {
    ...
    // when rx fifo has filled the bit timing has not fully completed.
    //   Just wait until we reach the waiting state 
    waiting_addr = offset+onewire_offset_waiting;
    timeout = 2000;
    while( pio_sm_get_pc(pio, sm) != waiting_addr ){
        sleep_us(1);
        if( timeout-- < 0 ){
            // FIXME: do something clever in case of timeout
        }
    }
...
// Transmit a byte and activate strong pullup after last bit has been sent.
//   Note: onewire_tx_byte_spu returns when the rx fifo has been read. This is 50 us prior to the end of the bit
//   and hence 50 us prior to the strong pullup actually activated.
//   Either consider this when controlling the strong pullup time or wait for idle before taking time. 
...
void onewire_tx_byte_spu(struct owobj *owobj, uint byte){
    ...
    pio->txf[sm] = byte;
    while( pio_sm_get_rx_fifo_level(pio, sm) == 0 ) ; // wait
    pio_sm_get(pio, sm); // read to drain RX fifo 
...
// This is code stolen from MAXIM AN3684, slightly modified to interface to the PIO onewire and to eliminate global variables. 
...
// Simple test with signle DS18B20 Write and readback scratchpad register 
...
// Simple test with single DS18B20 Configure, start conversion and read temperature Assume phantom power.
...
// Try testing with variable number of bits to be transmitted ; You should see 14 back-to-back high bits on the scope.
...
void onewire_temp_app(
    ...
    while( 1 ){
        ...
        s_id = 0;
        s_cnt = 0;
        // Search until we either have found all sensors or have passed the maximum possible sensor id 
        ...
        if( searching == 0 ){
            ...
            // Copy over from sdevs to tdevs. This is possible as tdevs does not store data which must persist over on scan loop. 
            for(c=0; c<N_CH; c++){
...
bool ds18b20Installed ;
uint8_t ds18b20Pin;
struct owobj owobj1;

void ds18b20Setup(void){  // load the pio program; init gpio; return true if ds18s20 is detected
    ds18b20Installed = false;
    if (ds18b20Pin == 255) return;
    owobj1.pio = pio1;
    owobj1.sm = 2;
    if ( ! pio_can_add_program(owobj1.pio, &onewire_program) ){
        printf("!! Pio1 does not have memory space to load ds18s20.pio program\n");
    }
    owobj1.offset = pio_add_program(owobj1.pio, &onewire_program);
    owobj1.pin = 15;
    onewire_program_init(owobj1.pio, owobj1.sm, owobj1.offset, owobj1.pin , owobj1.pin);
    pio_sm_set_enabled(owobj1.pio, owobj1.sm, true);

    if( ! onewire_reset(&owobj1) ){
        printf("No Device\n");
        return;
    }
    ds18b20Installed = true; // flag to say that a sensor is connected
...
void ds18b20Read(void){ 
    ...
    onewire_tx_byte(&owobj1, 0xcc); // Skip ROM command
    onewire_tx_byte_spu(&owobj1, 0x44); // Convert T
    sleep_ms(800);  /* 12bit: 750 ms */
    onewire_end_spu(&owobj1);
    ...
    onewire_tx_byte(&owobj1, 0xcc); // Skip ROM command
    onewire_tx_byte(&owobj1, 0xbe); // Read Scatchpad
    for(i=0; i<9; i++){
        buf[i] = onewire_rx_byte(&owobj1);
    }
...
int main() {
    ...
    // use sm 0-2 for blink test to learn if onwire code conflicts with this. 
