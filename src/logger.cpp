...
uint8_t logBuffer[LOG_BUFFER_LEN][2]; // 2 log buffers; this could be increased if UART is to slow and block other process
uint8_t logLen= 0;               // number of char in writing buffer
uint8_t logWritingBuffer = 0;         // buffer being currently use to write new data.

PIO loggerPio = pio1; // we use pio 0; DMA is hardcoded to use it
...
void LOGGER::begin(void ) {
    ...
    channel_config_set_dreq(&loggerDmaConfig, DREQ_PIO1_TX2);  // use PIO 0 and state machine 2 
    channel_config_set_transfer_data_size(&loggerDmaConfig, DMA_SIZE_8);
    dma_channel_configure(
        logger_dma_chan,
        &loggerDmaConfig,
        &pio1_hw->txf[0], // Write address (only need to set this once) of pio1 sm0
...
void LOGGER::logByteNoStuff(uint8_t c){
    ...
    logBuffer[logLen++][logWritingBuffer] = c; 
    if (logLen >=LOG_BUFFER_LEN) {
        ...
        dma_channel_set_trans_count (logger_dma_chan, LOG_BUFFER_LEN, true) ; // in principe DMA could already be started when changing the read address
        logWritingBuffer++;
