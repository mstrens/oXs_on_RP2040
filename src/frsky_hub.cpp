...
void sendFrskyHubFrame3(){  // gps date & time
    ...
    if (fields[GPS_TIME].onceAvailable )  {
        value =(fields[GPS_TIME].value >> 16) & 0XFFFF;// extract HHMM from original time format  HHMMSS00;
        sendHubValue(FRSKY_USERDATA_GPS_HM , value) ; // HOURMIN (hhmm)
        value=(fields[GPS_TIME].value ) & 0XFFFF;// extract SS00 from original time format  HHMMSS00;
